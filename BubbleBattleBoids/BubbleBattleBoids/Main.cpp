...
void Display()
{
    ...
    DrawText(Vec2(710.f, 585.f), 0.1f, timer);

    // draw state text
    switch (g_State)
    {
    case StateStart:
        DrawText(Vec2(130.f, 300.f), 0.3f, "Press (A) to Join");
        if (g_NumPlayers > 0)
            DrawText(Vec2(110.f, 250.f), 0.3f, "Press Start to Play");
        break;
    case StatePlay:
        break;
    case StateDone:
        DrawText(Vec2(270.f, 300.f), 0.3f, "Game Over");
        switch (g_Winner)
        {
        case Player::NNobody:
            break;
        case Player::NOne:
            DrawText(Vec2(270.f, 250.f), 0.2f, "Player One Wins!");
            break;
        case Player::NTwo:
            DrawText(Vec2(270.f, 250.f), 0.2f, "Player Two Wins!");
            break;
        case Player::NThree:
            DrawText(Vec2(270.f, 250.f), 0.2f, "Player Three Wins!");
            break;
        case Player::NFour:
            DrawText(Vec2(270.f, 250.f), 0.2f, "Player Four Wins!");
            break;
        }
        break;
    }
...
void Idle()
{
    ...
    g_FrameTime = (float)((now.QuadPart - g_LastFrame.QuadPart) / (double)g_Frequency.QuadPart);
    g_FrameTime = std::min(g_FrameTime, 2.f / 30.f);
    g_LastFrame = now;
...
void UpdatePhysics()
{
    ...
    for (GameObject* obj1 = GameObject::All(); obj1 != NULL; obj1 = obj1->GetNext())
        for (GameObject* obj2 = GameObject::All(); obj2 != NULL; obj2 = obj2->GetNext())
        {
            bool hit = false;
            ...
            if (obj1->GetShape() == GameObject::Circle && obj2->GetShape() == GameObject::Circle)
                hit = CircleCircleCollision(obj1->GetPosition(), 0.5f * obj1->GetScale(), obj2->GetPosition(), 0.5f * obj2->GetScale(), point, normal);
            else if (obj1->GetShape() == GameObject::Circle && obj2->GetShape() == GameObject::Rectangle)
                hit = CircleRectCollision(obj1->GetPosition(), 0.5f * obj1->GetScale(), obj2->GetPosition(), Vec2(0.5f, 0.5f) * obj2->GetScale(), point, normal);
            else
            {
                if (CircleRectCollision(obj2->GetPosition(), 0.5f * obj2->GetScale(), obj1->GetPosition(), Vec2(0.5f, 0.5f) * obj1->GetScale(), point, normal))
                {
                    hit = true;
                    normal = -normal;
                }
            ...
            if (hit)
            {
                ...
                obj2->Collision(obj2, point, normal);
            }
        }
}
